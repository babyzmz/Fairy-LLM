...
def test_public_method_manifest_matches_the_canonical_catalog() -> None:
    manifest_path = Path(__file__).resolve().parents[2] / "contracts" / "rpc-methods.json"
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    exported = {item["name"]: item for item in manifest["methods"]}

    assert manifest["schema_version"] == 1
    assert JsonRpcDispatcher.method_names() == frozenset(exported)
    assert exported == {
        name: {
            "name": name,
