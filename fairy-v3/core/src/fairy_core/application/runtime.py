...
_PREVIEW_POOL_CAPACITY = 3
_PREVIEW_IDLE_TIMEOUT = timedelta(minutes=10)
_EVICTABLE_TASK_STATUSES = frozenset({TaskStatus.READY, TaskStatus.ACCEPTED})
_ACTIVATABLE_TASK_STATUSES = frozenset(
    {
...
class RuntimeApplication(RuntimeApplicationSupport):
        ...
        with self._transaction() as (unit_of_work, _commands):
            ...
            if matching is not None:
                ...
                if matching.status is PreviewStatus.READY:
                    ...
                    return PreviewActivationResult(
                        ...
                        context=context,
                        adapter=adapter,
                        capacity=_PREVIEW_POOL_CAPACITY,
            ...
            for preview in active:
                ...
                if (
                    ...
                    or candidate_task.status not in _EVICTABLE_TASK_STATUSES
            ...
            victim = candidates[0] if len(active) >= _PREVIEW_POOL_CAPACITY and candidates else None
            if len(active) >= _PREVIEW_POOL_CAPACITY and victim is None:
                return PreviewActivationResult(
                    outcome=PreviewActivationOutcome.WAITING_FOR_SLOT,
                    context=None,
                    adapter=template.adapter.value,
            ...
            if task.status not in _ACTIVATABLE_TASK_STATUSES:
                return PreviewActivationResult(
                    outcome=PreviewActivationOutcome.WAITING_FOR_SLOT,
                    context=None,
                    adapter=template.adapter.value,
    ...
    def _reap_idle_previews(self, *, now: datetime) -> tuple[UUID, ...]:
        cutoff = now - _PREVIEW_IDLE_TIMEOUT
        with self._transaction() as (unit_of_work, _commands):
            state = unit_of_work.state
            candidates: list[_PoolVictim] = []
            for preview in state.active_previews():
                ...
                workspace = state.get_workspace(preview.workspace_id)
                if task is None or task.status not in _EVICTABLE_TASK_STATUSES or workspace is None:
                    continue
                candidates.append(
                    _PoolVictim(
                        preview=preview,
    ...
    @staticmethod
    def _runtime_adapter(runtime: RuntimeSession) -> str:
        return next(
            service.adapter
            for service in runtime.graph.services
            if service.service_id == runtime.graph.public_service_id
        )
    ...
    def _prepare_start(self, request: PreviewStartRequest) -> _StartIntent | PreviewContext:
        ...
        with self._transaction() as (unit_of_work, commands):
            ...
            if existing is not None:
                ...
                version = state.get_version(task.target_version_id)
                visibility = (
                    PreviewVisibility.PROJECT_ACTIVE
                    if version is not None
                    and version.visibility is VersionVisibility.PROJECT_ACTIVE
                    else PreviewVisibility.CHAT_DRAFT
                )
    ...
    def _finish_start(
        ...
        with self._transaction() as (unit_of_work, commands):
            ...
            state.save_task(task)
            self._set_active_preview(state, preview)
