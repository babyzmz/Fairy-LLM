...
def test_assistant_turn_work_migration_has_reversible_fenced_tenant_ddl() -> None:
    output = io.StringIO()
    config = Config(CLOUD_ROOT / "alembic.ini", output_buffer=output)

    command.downgrade(config, "20260716_0034:20260716_0033", sql=True)
...
def test_assistant_schedule_migration_has_reversible_tenant_ddl() -> None:
    output = io.StringIO()
    config = Config(CLOUD_ROOT / "alembic.ini", output_buffer=output)

    command.upgrade(config, "20260807_0047:20260807_0048", sql=True)

    upgrade_ddl = " ".join(output.getvalue().upper().split())
    assert "CREATE TABLE CORE_ASSISTANT_SCHEDULES" in upgrade_ddl
    assert "CREATE TABLE CORE_ASSISTANT_SCHEDULE_OCCURRENCES" in upgrade_ddl
    assert 'CREATE POLICY "TENANT_ISOLATION_CORE_ASSISTANT_SCHEDULES"' in upgrade_ddl

    output = io.StringIO()
    config = Config(CLOUD_ROOT / "alembic.ini", output_buffer=output)
    command.downgrade(config, "20260807_0048:20260807_0047", sql=True)
...
def test_message_cancellation_migration_has_tenant_policy_and_reverses() -> None:
    output = io.StringIO()
    config = Config(CLOUD_ROOT / "alembic.ini", output_buffer=output)
    command.upgrade(config, "20260909_0055:20260909_0056", sql=True)
    ddl = " ".join(output.getvalue().upper().split())
    assert "CREATE TABLE CORE_ASSISTANT_MESSAGE_CANCELLATIONS" in ddl
    assert "PRIMARY KEY (TENANT_ID, KEY_DIGEST)" in ddl
    assert '"CORE_ASSISTANT_MESSAGE_CANCELLATIONS" FORCE ROW LEVEL SECURITY' in ddl
