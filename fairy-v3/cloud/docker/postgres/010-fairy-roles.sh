...
psql \
  ...
  --set=worker_password="$FAIRY_WORKER_POSTGRES_PASSWORD" <<'SQL'
SELECT format(
...
GRANT CONNECT ON DATABASE :"database_name" TO fairy_app, fairy_worker;
GRANT USAGE ON SCHEMA public TO fairy_app, fairy_worker;
