...
2026-09-12 续接：按用户新要求，Phase 6–8 剩余代码先集中实现，中间不运行测试。
本轮多目标文件计划、音频焦点/空闲释放、模型启动预留、DDA 所有权、诊断与开发维护
的实现和最终验证记录见 [集中收尾记录](recovery-final-implementation-pass.md)。
本轮默认引擎已切到 4，并通过完整 Core 回归；存量 Turn 仍按原引擎收尾。
旧结果保留，真实 Provider/硬件与完整原生交互仍单列为未验收，不以自动化替代。
...
| 5 | 空闲退避、批量查询、有限历史、Browser 资源预算 | 主体实现及定向性能/真实无头Browser门禁通过；残余查询与最终联合验收待闭环 |
| 6 | 新版真实模型/工具节点，全局并发与恢复 | 代码闭环并默认版本4；全 Core 1368通过/4明确跳过；真实Provider/WSL仍待验收 |
| 7 | 音频焦点、模型空闲释放、资源预留、DDA 生命周期 | 已实现；真实进程回收和隐藏窗口资源测试已运行；原生GPU/录音/远程画质联合验收仍不完整 |
| 8 | 全面联合验收、诊断、架构与恢复文档、缓存维护 | 功能与最终本地自动回归完成；原生桌宠请求/合成冲突已修复；用户 Escape 停止界面复验，外部及剩余原生门禁未验收 |
...
### Phase 6–8：2026-09-12 集中实现与本地收尾

- Phase 6 `cdddb2e47`：新引擎默认4、多目标文件计划/审批/检查点、证据恢复与迁移，Core 1368通过/4跳过。
- 原生独立修复 `298a2e022`、`77e91591a`：桌宠宿主请求ID契约、输入玻璃与WebView2合成目标冲突。
- Phase 7 `fc0d3f8e2`：统一音频焦点、空闲5分钟释放、启动资源预留、DDA属性所有权与进程回收。
- TypeScript、Vitest 668项、Capabilities 120项、Cloud本地138项、Voice 19项、Rust全workspace、Clippy、Playwright 85项及边界检查通过。完整命令与测试变更理由见集中收尾记录。
- 原生复验遇到用户物理Escape停止Computer Use，未继续操作界面；本轮Tauri/Vite实例已关闭，未发现项目进程残留。正式数据库未改写，保留一致性备份和独立迁移演练。
- 这是功能与本地门禁收尾，不宣称真实Provider、WSL、PostgreSQL/S3、实际5分钟VRAM释放和完整原生录屏交互已验收。下面保留早期逐步实施记录，涉及当时默认版本/未完成项的描述均为历史证据。

### Phase 6：普通 Scratch 文件交付保存

- 复现两套引擎生成无需Preview的README后最终失败：非运行时分支漏做Scratch检查点。最小修复在这条分支完成真实检查点，项目聊天仍保留显式Review。
- 失败注入验证检查点失败不返回成功、不提升候选Version。相关Core/执行/运行时/审批/目标回归 **97通过（96.13秒）**。
- 增加调用同一 `run_stdio` 的轻量原生Worker示例，未启动Tauri窗口。最终6项保存测试通过（13.75秒），其中两项使用真实Rust Worker＋Git并核对HEAD内文件内容；Worker全targets Clippy通过。详见[验收证据](scratch-text-checkpoint.md)。
- 未把开发FileSystem适配器的内容摘要当成Git提交，也未把这项后端证据当成WebView2、真实Provider或WSL验收。多阶段文件计划继续推进。

### Phase 6：修改后复审与逐目标工具身份

- 已复现并修复“交付完成收起所有工具”导致后续复审不能读文件，以及同参重读被当成同阶段重复调用的问题。复审只恢复当前只读意图允许的工具，不重新开放修改权限。
- Invocation 新增受控 objective index；参数哈希不变，命令与去重按版本＋目标隔离，初始目标的旧恢复键保持兼容。工具节点、汇合及暂停恢复检查同一目标绑定。
- SQLite/Cloud 0061 迁移保留旧数据；工具尚在模型检查点、节点未发布的崩溃窗口也能恢复身份。错误编号、错会话、冲突来源和部分Schema拒绝迁移，不删记录。
- Assistant/Workflow/Media/SQLite及文件计划联合 **541通过（482.01秒）**；Cloud离线迁移/部署 **44通过（3.62秒）**；Ruff通过。详见[专项证据](workflow-objective-boundaries.md)。真实PG/Provider/原生及实际桌面库未由此验收。
- 多次文件修改计划、Scratch最终保存及剩余联合门禁继续推进，版本4仍不默认开启。

### Phase 2/6：复合目标执行边界（首个可回滚增量）

- 已复现并修复“先分析再修改”在分析阶段提前暴露修改工具的问题。新版本4续接通过真实 objective begin/complete 节点围住模型、工具、汇合和验证；权限取根意图与当前目标交集，不因下一阶段允许修改而提前放权。
- 分析现有文件要求本阶段的实际内容回执；缺证据先纠正，不开放修改阶段。进度绑定 Run、Task、Conversation、解释版本和源消息，损坏或跨会话记录不退回宽权限。中间公开分析保留为事实，仅最后阶段生成最终消息。
- 相关策略/持久化/模型节点/控制回归91项通过；最终3个真实临时Workspace/审批/重启/损坏记录场景通过（8.82秒）。详见[复合目标验收](workflow-objective-boundaries.md)。脚本模型不代替真实Provider，未触碰实际桌面数据库。
- 这是分步基础，不是Phase 2/6完成：修改后的同参重读、多次修改计划身份、额外中断与Steering、联合资源门禁继续推进；默认引擎仍为3。
...
- 默认引擎仍为3；版本4剩余失败/超时、审批中Steering、复合objective与并发联合门禁尚未闭环，不以本轮结果宣称Phase 6完成。
...
- 未知副作用恢复失败基线：已批准操作实际执行但结果未落库，重启后版本3/4都会重新调用。现在只有尚未派发的QUEUED调用，或显式幂等的none/read操作可自动恢复；已RUNNING且结果未知的write/execute操作重新取得Command租约仅用于结算，不再调用执行器，公开`TOOL_RESULT_UNCERTAIN`并停止Turn。一般idempotent标记不等于外部结果核对API。
- 检查点读取开销收敛：实库1 MB旧结果，在claim、3次心跳和等待查询中重复反序列化19 MB。新增受tenant/Run/Node约束的单Run、单Node读取；活动节点只保留其自身快照，心跳不重读旧图，等待只在停稳时返回完整图。预算准入/升级新增明确返回Run的内部接口，原完整Snapshot接口保留；Assistant暂停检查、预算与指定草稿读取已接入。修复后同一窗口旧结果反序列化为0；Workflow全包及新Assistant节点66项通过（55.35秒），Ruff通过。此探针度量旧正文反序列化量，不等于端到端CPU/内存占用已完成实机验收。
- Unicode兼容失败基线：70万汉字低于既有100万字符回复上限，却因UTF-8检查点超过默认2 MiB而被误报Provider错误。可信模型节点现在显式使用最高8 MiB的检查点预算，普通Adapter默认2 MiB不变；超过8 MiB、零/负数/布尔/非整数预算均拒绝。预算不能由模型工具参数指定。长Unicode回复单次请求、原检查点边界、Review及纯聊天20项通过（19.97秒），Ruff通过。
- Core阻止对该错误直接retry；聊天以普通用户可见文字要求检查实际结果，不提供盲目重试按钮。周期计划首次遇到未知结果即转attention_required并暂停，不累计到第三次再停。新旧引擎重启、审批与计划30项通过（41.75秒），桌面聊天/Turn hook39项通过，TypeScript与Ruff通过。测试模拟外部操作已发生但未保存结果，无系统通知或其他真实副作用；测试进程已退出，剩余仅本轮前的5个Codex Node。领域可确认的Media恢复将通过其持久Job/Run核对，不通过重新发起付费生成恢复。

- 版本4已接入单轮模型、真实工具节点、有序join、验证与最终消息；审批等待释放节点Worker。读取由Kernel执行，不调用旧批次线程池。工具定义/依赖图与预存Invocation、模型Command完成在同一事务提交，节点完成回执丢失复用原图和结果。恢复中的旧意图调用保留为拒绝结果，不重新取得修改权限。
- 本次完整Assistant回归288项通过（196.72秒）；默认版本3与实验版本4共同验证。该结果不覆盖尚未接入的Review节点与后续原生/资源联合门禁。
- 新旧引擎并存、无效请求与验证重试、两读取逆序完成、模型/工具/最终确认丢失、既有审批契约及截图作用域/清零31项通过（40.28秒）；相关Ruff通过。截图仅内存转交，每Turn20 MiB、全局80 MiB，消费、旧Revision、任务清理及关闭时释放；没有将截图写入检查点。仅脚本Provider/测试执行器，不是原生Browser/模型视觉质量验收。
- 接线时发现并独立修复并发预算漏计，见[专项证据](workflow-budget-concurrency-recovery.md)，提交`83b847cfb`。版本4仍非默认；Review节点、未知副作用恢复决策、Media非阻塞等待、完整Steering和资源/全套门禁未完成，不能将上述定向测试当作Phase 6闭环。

- 版本4先以宿主构造参数显式 opt-in 验证，不增加用户设置或环境变量，也不默认启用。新 Run 绑定版本4，存量2/3不迁移；请求解释和路由后，按 Run 已消费模型预算冻结下一个模型轮次，避免 Steering 复用旧 Command 键。先验纯聊天的模型→验证→最终回复真实节点，以及模型检查点/最终业务提交后的回执丢失恢复，再接工具、Review、Steering和领域等待。未完成这些门禁之前不切默认。

- 模型轮次切分契约：新引擎使用显式单轮边界；生成有效工具请求、最终草稿或需重试的无效请求时立即交回 Kernel，不调用工具、不创建最终 Message、不在当前节点继续请求下一轮模型。旧引擎无边界调用保持现有行为。边界回调意外返回必须失败关闭，不能退回旧工具循环；正常暂停/取消和 Provider 错误保留既有公开状态映射。测试使用真实 Application/数据库及脚本 Provider，直接观察 Provider 请求次数、零工具副作用和最终消息尚未生成。
- 单轮边界5项定向通过；整个 Assistant 包264项通过（158.70秒），相关Ruff通过。旧引擎默认无边界路径仍通过全部既有断言；目前只是新增了可交回调度权的执行路径，真实节点 Controller 与默认版本切换尚未完成。测试产生的 Python/Node 已退出，仅剩本轮之前已运行的5个 Codex Node 进程，未启动原生 Fairy 或 Voice。

- 最终回复拆分前置：消息、Turn终态与最终节点检查点在同一 UoW 提交；节点完成回执丢失后允许幂等恢复，但不能再插入消息或扣预算。过期 Fence、取消和另一 Turn 的 Claim 必须在业务写入前拒绝。验证使用独立测试数据库，关闭自身 Kernel 后手动推进真实节点 Claim，模拟业务提交后、节点完成前中断，保留已有模型 Command 的审批与租约边界。
- 已支持受 Fence 保护的最终业务提交；检查点绑定 Turn、模型 Command、正文 SHA-256、用量及证据引用。业务提交后重领不会重复追加消息，冲突正文拒绝，关闭重开后保持同一 Message ID；过期/取消/另一 Turn 的 Claim 均不能写回复。相关最终提交/Assistant应用/Workflow控制/检查点23项通过（30.33秒），Ruff通过；这项验证未调用真实 Provider，也不替代模型节点迁移。
...
- 已实现编译协议、预存 Invocation 原位执行，以及按 Invocation ID 精确恢复审批。工具执行仍复用原 Command Bus/意图/Scope/Policy/审批路径；默认未知资源锁整个 executor，只有可信 Adapter 可给出更精确域资源键。真实 Kernel 验证两个读取可同时 claim，写入屏障与跨 Run 账户资源互斥；关闭重开后有序 join 不提前解锁。
- 联合 Workflow/Assistant控制/工具分发/审批84项通过（71.42秒）；补充精确审批恢复后，工具批次/执行/分发/审批30项通过（30.96秒）。跨 Run 测试首版误复用了相同幂等键，导致第二个 Run 正确复用第一个；修正为按 Turn 唯一键后通过，没有更改生产幂等规则。新节点尚未作为默认 Assistant 调度入口，模型轮次和最终回复拆分仍未完成。
