...
function SettingsCategory(props: {
  ...
  switch (id) {
    ...
    case "pet": return <Category title="Pet" subtitle="Companion behavior">
      ...
      <SettingSelect icon={<ShieldCheck size={17} />} label="Glass privacy" detail={data.preferences.pet_optics_mode === "standard" ? "No desktop capture; uses a procedural optical environment" : "Locally samples only the pet area; never saves or uploads pixels"} value={data.preferences.pet_optics_mode} disabled={busy || data.preferences.pet_renderer_mode === "compatibility"} onChange={(value) => void updatePreferences({ pet_optics_mode: value as DesktopPreferences["pet_optics_mode"] })} options={[{ value: "standard", label: "Standard privacy" }, { value: "enhanced", label: "Enhanced refraction" }]} />
...
function SettingSelect({ icon, label, detail, value, options, disabled, onChange }: { icon: React.ReactNode; label: string; detail?: string; value: string; options: { value: string; label: string }[]; disabled: boolean; onChange(value: string): void }) {
